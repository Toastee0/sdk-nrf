...
#define SUPPORTED_MODES_COUNT 3 // grb, rgb, rgbw

#define DEVICES_MAX   1
#define DATA_PINS_MAX 1
...
BUILD_ASSERT(CONFIG_HPF_WS2812_MAX_RESPONSE_SIZE > 0, "Response max size should be greater that 0");

static const uint8_t pin_to_vio_map[HPF_WS2812_PINS_MAX] = {
		0,  /* Physical pin 0 */
};

static const hrt_xfer_bus_widths_t ws2812_bus_widths = {1, 1, 1, 1}; /* WS2812 uses single data line */
...
static volatile hpf_ws2812_timing_config_t ws2812_timing_config;
static volatile uint8_t ws2812_vio; /* VIO index used for WS2812 data out */
static volatile bool ws2812_enabled;
static volatile uint8_t ws2812_mode; /* 0..WS2812_MODES-1 */
static volatile hpf_ws2812_xfer_config_t hpf_ws2812_xfer_config;
static volatile hpf_ws2812_xfer_config_t *hpf_ws2812_xfer_config_ptr = &hpf_ws2812_xfer_config;

static volatile hrt_xfer_t xfer_params;

static volatile uint8_t response_buffer[CONFIG_HPF_WS2812_MAX_RESPONSE_SIZE];

static struct ipc_ept ep;
static atomic_t ipc_atomic_sem = ATOMIC_INIT(0);
#if defined(CONFIG_HPF_WS2812_FAULT_TIMER)
...
/*
 * WS2812 packer: pack GRB bytes into a stream of VPR ticks.
 * Each WS2812 bit is represented by `ticks_per_bit` samples (ticks).
 * For a logical '1' we emit `t1h_ticks` ones followed by zeros; for '0' we emit
 * `t0h_ticks` ones followed by zeros. Caller must provide reset_ticks to append
 * trailing low ticks for latch (>50us typical).
 *
 * Returns a k_malloc'd buffer of `*out_words` 32-bit words (caller must k_free).
 */
static uint32_t *ws2812_pack_grb(const uint8_t *grb, size_t leds, int ticks_per_bit,
								int t0h_ticks, int t1h_ticks, int reset_ticks,
								size_t *out_words)
{
	const int bits_per_led = 24; /* GRB */
	uint32_t total_ticks = leds * bits_per_led * ticks_per_bit + reset_ticks;
	size_t words = (total_ticks + BITS_IN_WORD - 1) / BITS_IN_WORD;
	uint32_t *buf = k_malloc(words * sizeof(uint32_t));
	if (buf == NULL) {
		*out_words = 0;
		return NULL;
	}
	...
	for (size_t i = 0; i < leds; i++) {
		/* WS2812 expects G,R,B order per LED */
		const uint8_t *p = &grb[i * 3];
		for (int c = 0; c < 3; c++) {
			uint8_t byte = p[c];
			for (int b = 7; b >= 0; b--) {
				int bit = (byte >> b) & 1;
				int high = bit ? t1h_ticks : t0h_ticks;
				for (int h = 0; h < high; h++) {
					size_t wi = bit_pos / BITS_IN_WORD;
					unsigned bi = bit_pos % BITS_IN_WORD;
					buf[wi] |= (1u << (BITS_IN_WORD - 1 - bi));
					bit_pos++;
				}
				bit_pos += (ticks_per_bit - high);
			}
		}
	}

	/* append reset (low) ticks */
	bit_pos += reset_ticks;
...
/* Prepare xfer_params and send packed words via existing HRT engine.
 * counter_value: VPR timer top value to set tick duration (platform-specific).
 * ticks: number of VPR ticks represented by the packed buffer (data_length).
 */
static int ws2812_send_packed(uint32_t *packed_words, size_t words, uint32_t ticks,
							 uint16_t counter_value)
{
	if (packed_words == NULL || words == 0) {
		return -EINVAL;
	}

	/* Configure global xfer_params for single-wire WS2812 output */
	xfer_params.counter_value = counter_value;
	xfer_params.ce_vio = CE_PIN_UNUSED;
	xfer_params.ce_hold = false;
	xfer_params.cpp_mode = MSPI_CPP_MODE_0;
	xfer_params.ce_polarity = MSPI_CE_ACTIVE_HIGH;
	xfer_params.bus_widths = (hrt_xfer_bus_widths_t){1, 1, 1, 1};

	/* Zero other phases */
	xfer_params.xfer_data[HRT_FE_COMMAND].data = NULL;
	xfer_params.xfer_data[HRT_FE_COMMAND].word_count = 0;
	xfer_params.xfer_data[HRT_FE_ADDRESS].data = NULL;
	...
	/* Data phase */
	xfer_params.xfer_data[HRT_FE_DATA].fun_out = HRT_FUN_OUT_WORD;
	xfer_params.xfer_data[HRT_FE_DATA].data = packed_words;

	/* data_length is number of VPR clocks (ticks) */
	adjust_tail(&xfer_params.xfer_data[HRT_FE_DATA], xfer_params.bus_widths.data, ticks);

	/* Ensure configuration visible before HRT */
	nrf_barrier_rw();

	/* Directly call hrt_write to emit the stream */
	hrt_write(&xfer_params);

	return 0;
}

#if defined(CONFIG_HPF_WS2812_DEMO)
/* Simple demo: send one green pixel (GRB) once on boot. Counter value is left
 * as a weak default and may require calibration on target hardware.
 */
static void run_ws2812_demo(void)
{
	const uint8_t led[3] = {0xFF, 0x00, 0x00}; /* G,R,B -> green */
	size_t words;
	/* ticks_per_bit=10, T0H=3, T1H=6, reset_ticks=400 (approx 50us @125ns) */
	uint32_t *buf = ws2812_pack_grb(led, 1, 10, 3, 6, 400, &words);
	if (buf == NULL) {
		return;
	}

	/* Choose a conservative counter value; calibrate on HW. */
	uint16_t counter_value = 2;
	uint32_t total_ticks = 1 * 24 * 10;

	ws2812_send_packed(buf, words, total_ticks, counter_value);
...
static void config_pins(hpf_mspi_pinctrl_soc_pin_msg_t *pins_cfg)
{
	...
	/* Choose a single VIO for WS2812 output (first available data_vios). */
	ws2812_enabled = false;
	for (uint8_t i = 0; i < DATA_PINS_MAX; i++) {
		if (data_vios[i] != DATA_PIN_UNUSED) {
			ws2812_vio = data_vios[i];
			ws2812_enabled = true;
			/* Restrict TX mask to only the WS2812 VIO. */
			xfer_params.tx_direction_mask = 0;
			WRITE_BIT(xfer_params.tx_direction_mask, ws2812_vio, VPRCSR_NORDIC_DIR_OUTPUT);
			xfer_params.rx_direction_mask = 0; /* no RX for WS2812 */
			break;
		}
	}
...
{
	atomic_set_bit(&ipc_atomic_sem, HPF_WS2812_EP_BOUNDED);
}

static void ep_recv(const void *data, size_t len, void *priv)
{
#ifdef CONFIG_HPF_WS2812_IPC_NO_COPY
	data = *(void **)data;
#endif
	...
	switch (opcode) {
#if defined(CONFIG_HPF_WS2812_FAULT_TIMER)
	case HPF_WS2812_CONFIG_TIMER_PTR: {
		const hpf_ws2812_flpr_timer_msg_t *timer_data =
			(const hpf_ws2812_flpr_timer_msg_t *)data;

		fault_timer = timer_data->timer_ptr;
		break;
	}
#endif
	case HPF_WS2812_CONFIG_PINS: {
		hpf_ws2812_pinctrl_soc_pin_msg_t *pins_cfg = (hpf_ws2812_pinctrl_soc_pin_msg_t *)data;
