			  ...
			  struct mbox_msg *msg_data)
{
	...
	hpf_ws2812_mbox_data_t *rx_data = (hpf_ws2812_mbox_data_t *)user_data;
...
int backend_init(backend_callback_t callback)
{
	...
	static hpf_ws2812_mbox_data_t *rx_data =
		(hpf_ws2812_mbox_data_t *)((uint8_t *)(DT_REG_ADDR(DT_NODELABEL(sram_rx))));
